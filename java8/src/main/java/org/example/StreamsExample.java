...
public class StreamsExample {
    ...
    public static void main(final String[] args) {
        ...
        // TODO With functional interfaces declared
        var allBooksFunction = new Function<Author, Stream<Book>>(){
            @Override
            public Stream<Book> apply(Author author) {
                return author.books.stream();
            }
        };
        var publishedBooksPredicate = new Predicate<Book>(){
        ...
        var booksInformation = new Consumer<Book>(){
        ...
        authors
                .stream()
                .flatMap(allBooksFunction)
                .filter(publishedBooksPredicate)
                .forEach(booksInformation);
        ...
        // TODO With functional interfaces declared
        var bookPriceFunction = new ToDoubleFunction<Book>() {
        ...
        var booksAvgPrice1 = authors.stream()
                .flatMap(allBooksFunction)
                .mapToDouble(bookPriceFunction)
                .average()
                .orElse(0.0);
        System.out.println(booksAvgPrice1);
        banner("Average price for all books in the library - lambda");
        // TODO With functional interfaces used directly
        var booksAvgPrice2 = authors.stream()
                .flatMap(author -> author.books.stream())
                .mapToDouble(book -> book.price)
                .average()
                .orElse(0.0);
        System.out.println(booksAvgPrice2);
        banner("Active authors that have at least one published book");
        // TODO With functional interfaces declared
        var publishedBookPredicate = new Predicate<Book>() {
            @Override
            public boolean test(Book book) {
                return book.published;
            }
        };
        var authorWithOnePublishedBookOrMore = new Predicate<Author>() {
            @Override
            public boolean test(Author author) {
        ...
        authors.stream()
                .filter(activeAuthorPredicate.and(authorWithOnePublishedBookOrMore))
                .forEach(authorPrintConsumer);
